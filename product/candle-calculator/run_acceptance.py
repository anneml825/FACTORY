...
YC, CP = "Candle", "Costs & Price"
OZ = 28.349523125

# Named so a layout change is a one-line edit, not a search-and-replace.
FILL, WAX, FO, TOTAL, LOAD, CONTENT = "B9", "B18", "B19", "B20", "B21", "B22"
MAXWARN, WAXBATCH = "B15", "B27"
COST, PRICE, PRICEWARN = "B34", "B48", "B49"
MARGIN, WHOLESALE, WSWARN = "B53", "B58", "B60"
...
wax, fo, fill, content = (e.get(YC, WAX), e.get(YC, FO), e.get(YC, TOTAL), e.get(YC, CONTENT))
...
wax3, fo3 = e3.get(YC, WAX), e3.get(YC, FO)
...
check("T4", "water weight 300 with factor 0.86 gives fill 258",
      close(e4.get(YC, FILL), 258), f"fill={e4.get(YC, FILL):.4f}")

ea, eb = ev(tests.TESTS["T5a"], "T5a"), ev(tests.TESTS["T5b"], "T5b")
wax_g, wax_oz = ea.get(YC, WAX), eb.get(YC, WAX)
cost_g, cost_oz = ea.get(CP, COST), eb.get(CP, COST)
...
cost6, price6, margin6 = e6.get(CP, COST), e6.get(CP, PRICE), e6.get(CP, MARGIN)
check("T6", "cost 5.00, fixed 0.45, fees 9.5%, target 40% gives price 10.79 and achieved 40.0%",
...
price7, warn7 = e7.get(CP, PRICE), e7.get(CP, PRICEWARN)
check("T7", "a 91% target with 9.5% fees is refused with no number in the price cell",
      price7 == "" and isinstance(warn7, str) and "not reachable" in warn7,
...
probs8 = e8.scan()
blanks = [e8.get(YC, WAX), e8.get(YC, FO), e8.get(CP, COST), e8.get(CP, PRICE)]
...
warn9, wax9 = e9.get(YC, MAXWARN), e9.get(YC, WAX)
...
ws10, cost10, warn10 = e10.get(CP, WHOLESALE), e10.get(CP, COST), e10.get(CP, WSWARN)
check("T10", "a wholesale price below unit cost warns",
      ws10 < cost10 and isinstance(warn10, str) and "below your cost" in warn10,
      f"wholesale={ws10:.2f} cost={cost10:.2f} message={warn10!r}")

mismatch = []
CELLS = {"fill": (YC, FILL), "wax": (YC, WAX), "fo": (YC, FO),
         "load": (YC, LOAD), "content": (YC, CONTENT), "unit_cost": (CP, COST)}
for name, inputs in tests.TESTS.items():
    build(f"dist/tests/{name}.xlsx", inputs)
    ec = Eval(f"dist/tests/{name}.xlsx")
    m = tests.model(inputs)
    for key, (sheet, addr) in CELLS.items():
        got, want = ec.get(sheet, addr), m[key]
        if want is not None and not close(got, want, 1e-9):
            mismatch.append(f"{name}.{key}: workbook={got} model={want}")
    if m["price"] is not None and not close(ec.get(CP, PRICE), m["price"], 1e-9):
        mismatch.append(f"{name}.price: workbook={ec.get(CP, PRICE)} model={m['price']}")

print(f"{'test':5} {'result':6} description")
print("-" * 100)
for n, rres, d, det in results:
    print(f"{n:5} {rres:6} {d}")
    print(f"{'':12} {det}")
print()
