...
CREATE TYPE owner_labor_kind AS ENUM (
    ...
    'APPROVAL',           -- batched decisions
    'EXCEPTION'           -- rare KYC, dispute, legal, provider, or technical intervention
);
...
CREATE TABLE arrival_publication (
    id                      BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
-- Raw signed events are the durable source of truth. Snapshots are rebuilt by replay.
CREATE TABLE watch_event_inbox (
...
CREATE INDEX watch_event_experiment_idx
...
CREATE TABLE stripe_transaction_reference (
